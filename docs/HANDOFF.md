...
| **Characters and story archive** (`ui/characters_screen.dart`) | A fourth main tab lists all five core characters. Each profile shows identity, role, element, Bond, ascension and all seven route beats. Completed beats replay through a cloned `GameState`, so replay choices cannot farm Bond, replace saved flags or retrigger ascension. |
