...
class DialogueScreen extends StatefulWidget {
  ...
  /// Replays a completed beat without changing Bond, flags, completion, or
  /// ascension. Choices still branch normally inside the replay because the
  /// dialogue engine receives a disposable copy of the current save state.
...
class _DialogueScreenState extends State<DialogueScreen> {
  ...
  void _start(Scene scene) {
    final engine = DialogueEngine(
      scene: scene,
      state: widget.isReplay
          ? GameState.fromJson(widget.game.state.toJson())
          : widget.game.state,
  ...
  Widget _continueBar(bool atEnd) => Container(
        ...
        child: SlabButton(
          atEnd
              ? (widget.isReplay ? 'Close replay' : 'End of scene')
              : 'Continue',
          filled: true,
