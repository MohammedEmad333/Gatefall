// Widget smoke tests: the app boots, the four tabs render, and a scene can
...
void main() {
  ...
  testWidgets('all four tabs render without throwing', (tester) async {
    ...
    expect(find.text('Everyone drawn to the house, and every story you have shared.'),
        findsOneWidget);
  ...
  testWidgets('character archive replays a story without changing its choice',
      (tester) async {
    final game = await pumpGame(tester, setup: (g) {
      ...
      g.state.flags['FAELEN_APPROACH'] = 'patience';
    ...
    expect(find.text('Elf, ex-Warden · Warden'), findsOneWidget);
    await tester.tap(find.byKey(const Key('replay-faelen_b1_the_wall')));
    await tester.pumpAndSettle();
    ...
    await tester.tap(find.text('"You don\'t have to hide that you care."'));
    await tester.pumpAndSettle();

    // The replay can follow another branch, but the real save remembers the
    // choice and Bond earned on the original playthrough.
    expect(game.state.flags['FAELEN_APPROACH'], 'patience');
    expect(game.bondPoints('faelen'), 100);
    ...
    await tester.tap(find.text('Close replay'));
    await tester.pumpAndSettle();

    expect(game.state.flags['FAELEN_APPROACH'], 'patience');
    expect(game.bondPoints('faelen'), 100);
    expect(game.state.completedBeats,
